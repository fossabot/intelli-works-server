...
class TokenOP (base64UrlSafeKey: String) {
    private val logger = LoggerFactory.getLogger(TokenOP::class.java)
    private val keySpec: SecretKeySpec = SecretKeySpec(decodeBase64(base64UrlSafeKey), "AES")
    ...
    private fun generateToken(epoch: Long): String? {
        val epochInMinute = cutEpochToMinute(epoch)

        val input = longToBytes(epochInMinute)

        val iv = generateIv(bytesToLong(Arrays.copyOfRange(keySpec.encoded, 0, 8)), epochInMinute)
    ...
    private fun validateToken(epoch: Long, cipherText: ByteArray): Boolean {
        val baseEpoch = cutEpochToMinute(epoch)
    ...
    fun generateCode(counter: Int): String? {
        val input = longToBytes(counter.toLong())

        val iv = generateIv(bytesToLong(Arrays.copyOfRange(keySpec.encoded, 0, 8)), bytesToLong(Arrays.copyOfRange(keySpec.encoded, 8, 16)))
        val spec = GCMParameterSpec(128, iv)
    ...
    fun validateCode(currentCounter: Int, code: String): Boolean {
        ...
        return try {
            ...
            val plainText = cipher.doFinal(decodeBase64(code))
            val codeCounter = bytesToLong(plainText)
    ...
    private fun parseToken(epoch: Long, cipherText: ByteArray): Long {
        val iv = generateIv(bytesToLong(Arrays.copyOfRange(keySpec.encoded, 0, 8)), epoch)
        val spec = GCMParameterSpec(128, iv)
